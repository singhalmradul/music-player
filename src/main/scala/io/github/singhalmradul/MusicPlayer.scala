...
class MusicPlayer extends Application:
  ...
  private var vb: VBox = uninitialized
  private var vb2: VBox = uninitialized
  private var stage: Stage = uninitialized
  private var openFile: MenuItem = uninitialized
  private var openFolder: MenuItem = uninitialized
  private var stageTitle: Label = uninitialized
  private var title: Label = uninitialized
  private var artist: Label = uninitialized
  private var album: Label = uninitialized
  private var currentDuration: Label = uninitialized
  private var totalDuration: Label = uninitialized
  private var volumeValue: Label = uninitialized
  private var repeatTrack: Label = uninitialized
  private var toggleRepeat: JFXToggleButton = uninitialized
  private var songSlider: JFXSlider = uninitialized
  private var volumeSlider: JFXSlider = uninitialized
  private var logo: JFXButton = uninitialized
  private var bt: JFXButton = uninitialized
  private var minimize: JFXButton = uninitialized
  private var exit: JFXButton = uninitialized
  private var previousButton: JFXButton = uninitialized
  private var playButton: JFXButton = uninitialized
  private var pauseButton: JFXButton = uninitialized
  private var nextButton: JFXButton = uninitialized
  private var mute: JFXButton = uninitialized
  private var volume: JFXButton = uninitialized
  private var artwork: ImageView = uninitialized
  private var volumeIcon: ImageView = uninitialized
  private var hb7: HBox = uninitialized
  private var hb: HBox = uninitialized
  private var hb1: HBox = uninitialized
  private var hb2: HBox = uninitialized
  private var hb3: HBox = uninitialized
  private var mediaPlayer: MediaPlayer = uninitialized
  private var cm: ContextMenu = uninitialized
  private var window: BorderPane = uninitialized
  private var bp: BorderPane = uninitialized
  private var bp2: BorderPane = uninitialized
  ...
  def getResource(location: String): InputStream =
    classOf[MusicPlayer].getClassLoader().getResourceAsStream(location)
  ...
  override def start(stage: Stage): Unit =
    ...
    try {
      ...
      stage.getIcons.add(
        new Image(getResource("static/assets/logo.png"), 32, 32, true, true)
      )
  ...
  private def showInfo(path: String): Unit =

    val mp3file = new Mp3File(new File(path));
    var albumArtwork = "";
    ...
    if mp3file.hasId3v2Tag then {
      ...
      if imageData != null then
        ...
        albumArtwork = "static/assets/album-artwork." + id3v2Tag
          .getAlbumImageMimeType()
          .substring(6);
        val file = new RandomAccessFile(albumArtwork, "rw");
        file.write(imageData);
        file.close();

        try {
          artwork.setImage(
            new Image(getResource(albumArtwork), 100, 100, true, true)
          );
        } catch {
          case e: Exception =>
            artwork.setImage(
              new Image(
                getResource("static/assets/louis-tomlinson.png"),
                100,
                100,
                true,
                true
  ...
  private def getMainPlayer(): Scene =
    new Scene(mainPlayerSetup())

  private def mainPlayerSetup(): Parent =
    songSlider = new JFXSlider(0, 100, 0);
    songSlider.setDisable(true);
    songSlider.setValueFactory(event =>
    ...
    songSlider.setOnMousePressed(event =>
      mediaPlayer.seek(Duration.seconds(songSlider.getValue()))
    );
    songSlider.setOnMouseDragged(event =>
      mediaPlayer.seek(Duration.seconds(songSlider.getValue()))
    );
    ...
    logo = new JFXButton(
      "",
      new ImageView(
        new Image(getResource("static/assets/logo.png"), 24, 24, true, true)
      )
    );
    ...
    openFile = new MenuItem(
      "open file",
      new ImageView(
        new Image(getResource("static/assets/note.png"), 24, 24, true, true)
      )
    );
    openFile.setOnAction(e => {
      val fc = new FileChooser();
      val fl = fc.showOpenDialog(stage);
      if fl != null then
    ...
    openFolder = new MenuItem(
      "open folder",
      new ImageView(
        new Image(
          getResource("static/assets/music-folder.png"),
          24,
          24,
          true,
          true
    ...
    bt = new JFXButton(
      ...
      new ImageView(
        new Image(
          getResource("static/assets/miniPlayer.png"),
          16,
          16,
          true,
          true
    ...
    minimize = new JFXButton(
      "",
      new ImageView(
        new Image(getResource("static/assets/minimize.png"), 16, 16, true, true)
      )
    );
    ...
    exit = new JFXButton(
      "",
      new ImageView(
        new Image(getResource("static/assets/cancel.png"), 16, 16, true, true)
      )
    );
    ...
    previousButton = new JFXButton(
      ...
      new ImageView(
        new Image(
          getResource("static/assets/back-arrows.png"),
          32,
          32,
          true,
          true
    ...
    playButton = new JFXButton(
      "",
      new ImageView(
        new Image(getResource("static/assets/play.png"), 32, 32, true, true)
      )
    );
    ...
    pauseButton = new JFXButton(
      "",
      new ImageView(
        new Image(getResource("static/assets/pause.png"), 32, 32, true, true)
      )
    );
    ...
    nextButton = new JFXButton(
      ...
      new ImageView(
        new Image(
          getResource("static/assets/forward-arrows.png"),
          32,
          32,
          true,
          true
    ...
    mute = new JFXButton(
      "",
      new ImageView(
        new Image(getResource("static/assets/mute.png"), 32, 32, true, true)
      )
    );
    mute.setOnAction(event => {
      if lastVolume == 0 then volumeSlider.setValue(100);
      else volumeSlider.setValue(lastVolume);
    });

    volumeIcon = new ImageView(
      new Image(getResource("static/assets/speaker.png"), 32, 32, true, true)
    );

    volume = new JFXButton("", volumeIcon);
    volume.setOnAction(event => {
      val t = volumeSlider.getValue().toInt
  ...
  private def getSongList(
      files: ArrayList[String],
      fl: File
  ): ArrayList[String] =

    for file <- fl.listFiles() do

      val name = file.getName();

      if name.endsWith(".mp3") || name.endsWith(".m4a") || name.endsWith(".wav")
      then files.add(file.getAbsolutePath());
      else if file.isDirectory() then getSongList(files, file);

    files;

  private def volumeHandler(): Unit =

    if mediaPlayer != null then
      mediaPlayer.setVolume(volumeSlider.getValue() / 100);

    lastVolume = Math.ceil(volumeSlider.getValue()).toInt
    volumeValue.setText(String.valueOf(lastVolume));

    if volumeSlider.getValue() == 0 then hb2.getChildren().set(0, mute);
    else
      try {

        if volumeSlider.getValue() < 25 then
          volumeIcon.setImage(
            new Image(
              getResource("static/assets/speaker1.png"),
              32,
              32,
              true,
              true
        ...
        else if volumeSlider.getValue() < 50 then
          volumeIcon.setImage(
            new Image(
              getResource("static/assets/speaker2.png"),
              32,
              32,
              true,
              true
        ...
        else if volumeSlider.getValue() < 75 then
          volumeIcon.setImage(
            new Image(
              getResource("static/assets/speaker3.png"),
              32,
              32,
              true,
              true
          ...
          volumeIcon.setImage(
            new Image(
              getResource("static/assets/speaker.png"),
              32,
              32,
              true,
              true
  ...
  private def getFormattedDuration(duration: Long): String =
    duration / 60 + (if (duration % 60) < 10 then ":0"
                     else ":") + duration % 60;

  private def startPlaying(ar: ArrayList[String]): Unit =
    var q = ar.listIterator();
    if q.hasNext() then startPlaying(q.next());

    mediaPlayer.setOnEndOfMedia(() => {
      if q.hasNext() then startPlaying(q.next());
      else if toggleRepeat.isSelected() then startPlaying(ar);
      else stopPlaying();
    });
  ...
  private def startPlaying(path: String): Unit =

    if mediaPlayer != null then mediaPlayer.stop();
    ...
    mediaPlayer
      ...
      .addListener((observable, oldValue, newValue) => {
        ...
        currentDuration
          .setText(getFormattedDuration((newValue.toSeconds.toLong)));
      });
    mediaPlayer.setOnReady(() => {
      ...
      totalDuration.setText(
        getFormattedDuration(
          (mediaPlayer.getMedia.getDuration.toSeconds.toLong)
        )
      )
      mediaPlayer.setVolume(volumeSlider.getValue() / 100);
  ...
  private def play(): Unit =
    ...
    if mediaPlayer != null then
      ...
      hb1.getChildren().set(1, pauseButton);
...
object MusicPlayer:
  def main(args: Array[String]): Unit =
    Application.launch(classOf[MusicPlayer], args*)
